...
const AISDKInsightsPage = async () => {
  const summary = await getLatestSummary('aisdk');
    ...
    <div className="container mx-auto px-4 py-12 bg-gray-900 text-white min-h-screen">
      ...
      {summary ? (
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-semibold mb-4 text-blue-300">Latest AI SDK Summary</h2>
          <div className="prose prose-invert max-w-none">
            <ReactMarkdown 
              ...
              components={{
                ...
                code: ({ inline, className, children, ...props }: React.PropsWithChildren<{
                  inline?: boolean;
                  className?: string;
                }>) => 
                  inline 
                    ? <code className={`bg-gray-700 text-pink-300 px-1 rounded ${className || ''}`} {...props}>{children}</code>
                    : <code className={`block bg-gray-700 p-2 rounded my-2 text-pink-300 ${className || ''}`} {...props}>{children}</code>,
              }}
            >
              {summary.summary}
            </ReactMarkdown>
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
          <p className="text-gray-300">No summary available for AI SDK.</p>
        </div>
      )}
    </div>
  );
